...
public class ReceptKompDAO implements IReceptKompDAO {
	...
	public ReceptKompDTO getReceptKomp(int receptkompId, int raavareId)
		...
		try
		{
			...
			while (rs.next()) 
			{
				...
				resReceptKomp = new ReceptKompDTO(rs.getInt("recept_id"), rs.getInt("raavare_id"), rs.getInt("nom_netto"), rs.getInt("tolerance"));
			}
		}
	...
	public List<ReceptKompDTO> getReceptKompList(int receptkompId)
		...
		try
		{
			while (rs.next()) 
			{
				list.add(new ReceptKompDTO(rs.getInt("recept_id"), rs.getInt("raavare_id"), rs.getInt("nom_netto"), rs.getInt("tolerance")));
			}
		}
		catch (SQLException e) { throw new DALException(e); }
		return list;
